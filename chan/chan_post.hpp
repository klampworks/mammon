...
struct chan_post {
	...
	chan_post(const std::string &board_p, const std::string &thread_id_p,
		...
		const std::string &img_url_p, const std::string &content_p) :
			...
			img(img_p), img_url(img_url_p), content(content_p) {}

	chan_post(const std::string &board_p, const std::string &thread_id_p,
		std::string &&post_id_p, std::string &&img_p, 
		std::string &&img_url_p, std::string &&content_p) :
			board(board_p), thread_id(thread_id_p), post_id(std::move(post_id_p)),
			img(std::move(img_p)), img_url(std::move(img_url_p)), 
